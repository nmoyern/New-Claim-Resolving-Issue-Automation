...
async def handle_mco_auth_check(claim: Claim) -> ResolutionResult:
    ...
    lauris_auth = None
    try:
        # Look up consumer UID from Medicaid/record number
        async with LaurisSession() as lauris:
            consumer_uid = await _lookup_uid_from_record_number(
                lauris.page, claim.client_id
            )
        ...
        if consumer_uid:
            lauris_auth = await check_lauris_authorization(
                consumer_uid=consumer_uid,
                claim_dos=claim.dos,
                service_code=claim.proc_code or claim.service_code,
            )

            if lauris_auth and lauris_auth.get("found"):
                lauris_auth_num = lauris_auth["auth_number"]
                lauris_payor = lauris_auth["payor"]
                has_scanned = lauris_auth["has_scanned_doc"]
                suspicious = lauris_auth.get("suspicious", "")

                logger.info(
                    "Lauris auth found",
                    claim_id=claim.claim_id,
                    auth_number=lauris_auth_num,
                    payor=lauris_payor,
                    has_scanned_doc=has_scanned,
                )

                # Check if auth is already on the claim
                auth_already_on_claim = (
                    claim.auth_number
                    and claim.auth_number == lauris_auth_num
                )

                if auth_already_on_claim:
                    # Auth is on claim — verify dates match
                    logger.info(
                        "Lauris auth already on claim — verifying dates",
                        claim_id=claim.claim_id,
                        auth=lauris_auth_num,
                    )
                    note = (
                        f"Lauris auth #{lauris_auth_num} already on claim. "
                        f"Auth period: {lauris_auth['start_date']} - "
                        f"{lauris_auth['end_date']}. "
                        f"Payor: {lauris_payor}. "
                        f"Scanned doc: {'yes' if has_scanned else 'no'}."
                    )
                    if suspicious:
                        note += f" WARNING: {suspicious}"

                    # Auth on claim and dates match — PASS, proceed to
                    # MCO portal for reconsideration (Step 2 will handle)
                    # Fall through to MCO portal check
                else:
                    # Auth found in Lauris but NOT on claim → add to claim
                    # and resubmit
                    logger.info(
                        "Lauris auth found but not on claim — correcting",
                        claim_id=claim.claim_id,
                        auth=lauris_auth_num,
                    )
                    claim.auth_number = lauris_auth_num

                    corrections = {"auth_number": lauris_auth_num}
                    api = ClaimMDAPI()
                    note = (
                        f"Auth #{lauris_auth_num} found in Lauris "
                        f"(period: {lauris_auth['start_date']} - "
                        f"{lauris_auth['end_date']}, payor: {lauris_payor}). "
                        f"Added to claim and resubmitting."
                    )
                    if suspicious:
                        note += f" WARNING: {suspicious}"
                    ...
                    if api.key:
                        ...
                        async with ClaimMDSession() as claimmd:
                            success = await claimmd.correct_and_resubmit(
                                claim, corrections
                            )
            ...
            logger.warning(
                "Could not resolve Medicaid ID to Lauris UID — skipping Lauris auth check",
                claim_id=claim.claim_id,
                client_id=claim.client_id,
            )
    except Exception as e:
        logger.warning(
            "Lauris auth check failed — continuing to MCO portal",
            claim_id=claim.claim_id,
            error=str(e),
        )

    # -----------------------------------------------------------------------
    # Step 2+: MCO portal check and downstream cascade
    # -----------------------------------------------------------------------

    # United: check portal first, but if NOT found, flag ClickUp (never faxed)
    if claim.mco == MCO.UNITED:
        checker = get_auth_checker(MCO.UNITED)
        if checker:
            async with checker as portal:
                auth_found, auth_record = await portal.check_auth(claim)
                if auth_found and auth_record:
                    claim.auth_number = auth_record.auth_number
                    note = note_auth_verified_in_portal("United", auth_record.auth_number)
                    # Submit recon via TrackIt in the SAME session
        ...
        # Auth not found in United portal — United does NOT use fax
        return ResolutionResult(
            claim=claim,
            action_taken=ResolutionAction.MCO_PORTAL_AUTH_CHECK,
            success=False,
            needs_human=True,
            human_reason=(
                "United auth not found in portal. United does NOT use fax. "
                "Create urgent ClickUp to resend auth (assessor for initial stay; "
                "responsible person per spreadsheet for continued stay)."
            ),
        )
    ...
    if not checker:
        return ResolutionResult(
            ...
            success=False,
            needs_human=True,
            human_reason=f"No portal automation available for MCO: {claim.mco.value}",
        )
    ...
    try:
        ...
        logger.error("MCO portal check failed", mco=claim.mco.value, error=str(e))
        return ResolutionResult(
            claim=claim,
            action_taken=ResolutionAction.MCO_PORTAL_AUTH_CHECK,
            success=False,
            needs_human=True,
            human_reason=(
                f"MCO portal ({claim.mco.value}) login or auth check failed: {str(e)[:80]}. "
                f"May need to refresh portal session."
            ),
        )

    if auth_found and auth_record:
        # Special handling for EXCEEDED_UNITS: if the MCO portal shows
        # approved units > used units, submit reconsideration stating
        # we did not exceed (don't just call MCO).
        if (claim.denial_codes
                and DenialCode.EXCEEDED_UNITS in claim.denial_codes
                and auth_record.approved_units > 0):
            # We have the auth — check if units were truly exceeded
            note = note_auth_verified_in_portal(
                claim.mco.value, auth_record.auth_number
            )
            recon_note = (
                f"Exceeded units denial investigated. MCO portal shows "
                f"approved units: {auth_record.approved_units}. "
                f"We did not exceed authorized units. "
                f"Submitting reconsideration."
            )
            api = ClaimMDAPI()
            if api.key:
        ...
        # Auth exists in MCO portal.
        # Flow: FIRST correct the claim (add auth number, fix entity if needed)
        # and resubmit.  Only if that resubmission doesn't resolve it (claim
        # comes back denied again) do we escalate to reconsideration.
        # If the auth is already on the claim and all info is correct, go
        # straight to reconsideration.
        claim.auth_number = auth_record.auth_number
        note = note_auth_verified_in_portal(claim.mco.value, auth_record.auth_number)

        # Check whether the auth number is already on the claim
        auth_already_on_claim = (
            claim.last_note
            and auth_record.auth_number
            and auth_record.auth_number in claim.last_note
        )

        if auth_already_on_claim:
            # Auth is already on the claim — info was correct, go straight to recon
            logger.info(
                "Auth already on claim — submitting reconsideration directly",
                claim_id=claim.claim_id,
                auth=auth_record.auth_number,
            )
            api = ClaimMDAPI()
            if api.key:
                success = await api.submit_appeal(claim.claim_id, {
                ...
                if success:
                    recon_note = note_reconsideration_submitted(claim.mco.value)
                    await api.add_claim_note(claim.claim_id, recon_note)
            else:
                async with ClaimMDSession() as claimmd:
                    success = await claimmd.submit_reconsideration(claim)
            ...
            return ResolutionResult(
                ...
                action_taken=ResolutionAction.MCO_PORTAL_AUTH_CHECK,
                success=success,
                note_written=f"{note} Auth already on claim — reconsideration submitted.",
            )
        else:
            # Auth found but not yet on the claim — correct first, then resubmit
            logger.info(
                "Auth found in portal — correcting claim and resubmitting first",
                claim_id=claim.claim_id,
                auth=auth_record.auth_number,
            )
            corrections = {"auth_number": auth_record.auth_number}
            # Fix entity/billing region if the auth record indicates a different company
            if auth_record.program and auth_record.program != claim.program:
                from config.settings import ORG_KJLN, ORG_NHCS, ORG_MARYS_HOME
                from config.models import Program
                region_map = {
                    Program.KJLN: ORG_KJLN,
                    Program.NHCS: ORG_NHCS,
                    Program.MARYS_HOME: ORG_MARYS_HOME,
                }
                correct_region = region_map.get(auth_record.program)
                if correct_region:
                    corrections["billing_region"] = correct_region
            ...
            if api.key:
                ...
                if success:
                    correction_note = (
                        f"{note} Corrected claim: added auth {auth_record.auth_number}"
                        + (f", fixed entity to {corrections['billing_region']}" if "billing_region" in corrections else "")
                        + ". Resubmitted — if denied again, reconsideration will follow."
                    )
                    await api.add_claim_note(claim.claim_id, correction_note)
            else:
                async with ClaimMDSession() as claimmd:
                    success = await claimmd.correct_and_resubmit(claim, corrections)
