...
def _ensure_patient_tasks_table():
    ...
    try:
        conn.execute("ALTER TABLE clickup_patient_tasks ADD COLUMN patient_key TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_patient_task_key "
        "ON clickup_patient_tasks(patient_key)"
    )
    conn.execute(
...
class ClickUpTaskCreator:
    ...
    def get_existing_task_for_patient(
        self,
        patient_name: str,
        patient_key: str = "",
    ) -> Optional[str]:
        ...
        if patient_key:
            row = conn.execute(
                """SELECT task_id FROM clickup_patient_tasks
                   WHERE patient_key = ? AND status = 'open'
    ...
    async def create_or_update_patient_task(
        ...
        patient_key = patient_key or client_id or patient_name
        existing_task_id = self.get_existing_task_for_patient(patient_name, patient_key)
        ...
        if existing_task_id:
            ...
            comment = (
                ...
                f"What's needed:\n{needed or 'Manual review and resolution of the above issue(s).'}\n\n"
                f"Generated by Claims Automation on {today}."
            )
        ...
        # Create new task
        name = task_name or f"Claims Issue — {patient_name} [{today}]"
        client_id_line = f"Lauris Unique ID: {client_id}\n" if client_id else ""
        description = (
        ...
        if task_id:
            ...
            conn.execute(
                """INSERT INTO clickup_patient_tasks
                   (patient_key, patient_name, task_id, status, created_at, updated_at)
                   VALUES (?, ?, ?, 'open', ?, ?)""",
                (patient_key, patient_name, task_id, now, now),
            )
            conn.commit()
            conn.close()
            self.log.info(
